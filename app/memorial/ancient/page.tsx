...
export default function AncientMemorial() {
  ...
  const t = {
    'zh-TW': {
      title: "古代先賢堂",
      subtitle: "中華文明的根與魂",
      searchPlaceholder: "搜尋先賢（如 神農、女媧、孔子）",
      offeringButton: "線上祭祀",
      offeringTitle: "線上祭祀",
    ...
    'zh-CN': {
      title: "古代先贤堂",
      subtitle: "中华文明的根与魂",
      searchPlaceholder: "搜索先贤（如 神农、女娲、孔子）",
      offeringButton: "线上祭祀",
      offeringTitle: "线上祭祀",
    ...
    en: {
      ...
      success: "Offering completed. The sage has received your respect.",
  ...
  const [sages, setSages] = useState<Sage[]>([
    { id: 1, name: "神農氏", desc: "炎帝，神農氏，教民耕種、嘗百草、發明農業與醫藥", icon: "🌾", offerings: 0 },
    { id: 2, name: "女媧", desc: "女媧娘娘，補天造人，人類始祖", icon: "🌈", offerings: 0 },
    { id: 3, name: "炎帝", desc: "神農氏，教民耕種、嘗百草", icon: "🔥", offerings: 0 },
    { id: 4, name: "黃帝", desc: "中華人文始祖，發明舟車、醫藥、文字", icon: "👑", offerings: 0 },
    { id: 5, name: "孔子", desc: "儒家創始人，萬世師表", icon: "📖", offerings: 0 },
    { id: 6, name: "老子", desc: "道家創始人，《道德經》作者", icon: "🌀", offerings: 0 }
  ]);

  const filteredSages = sages.filter(s =>
    s.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    s.desc.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const handleOffering = (sage: Sage) => {
    setSelectedSage(sage);
    setShowOffering(true);
  };

  const completeOffering = () => {
    if (!selectedSage) return;

    setSages(prev => prev.map(s =>
      s.id === selectedSage.id ? { ...s, offerings: s.offerings + 1 } : s
    ));
      ...
      <div className="max-w-6xl mx-auto px-8 pb-24">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredSages.map((sage) => (
            <div key={sage.id} className="bg-[#2A1A12] p-10 rounded-3xl border border-[#C9A84C]/40 hover:border-[#C9A84C] transition-all text-center group">
              ...
              <p className="text-[#EDE0BA]/80 mb-8 leading-relaxed">{sage.desc}</p>

              <button 
                onClick={() => handleOffering(sage)}
              ...
              >
                🪔 {current.offeringButton} ({sage.offerings})
              </button>
            </div>
          ))}
        </div>
